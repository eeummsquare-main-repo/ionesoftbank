...
ElseIF idx="" OR Session("conPass")="" Then
...
Else
	...
	Else
		...
		IF DBpass <> Session("conPass") Then
			ErrorMsg = "RELOAD|비밀번호가 일치하지 않습니다."
		ElseIF status<>"0" Then
			ErrorMsg = "RELOAD|예약대기인 신청서만 수정 가능합니다."
		ElseIF dbRevDate < CStr( DateAdd("d",2,Date()) ) Then
			ErrorMsg = "RELOAD|예약일자가 2틀 후인 신청서만 수정 가능합니다."
		End IF
	End IF
End IF
