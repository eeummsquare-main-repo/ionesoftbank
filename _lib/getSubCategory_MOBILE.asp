...
IF catecode<>"" AND (depth="1" Or depth="2") Then
	IF depth="1" Then
		inputName = "catecode2"
		Sql="SELECT name, lowcode, dbo.fn_CateBarIMGM(lowcode) AS CateIMG From Category Where Topcode='"&catecode&"' AND lowcode=middlecode AND lowcode<>topcode Order By align1Num ASC,align2Num ASC,align3Num ASC"
	ElseIF depth="2" Then
		inputName = "catecode3"
		Sql="SELECT name, lowcode, dbo.fn_CateBarIMGM(lowcode) AS CateIMG From Category Where middlecode='"&catecode&"' AND lowcode<>middlecode AND lowcode<>topcode Order By align1Num ASC,align2Num ASC,align3Num ASC"
	End IF
