...
<% IF HK_RepYN = "True" And TopYn<>"True" AND PublicYN="False" AND HK_ImgYN<>"True" Then %>
	<!-- <input type="button" value="답변" onclick="<%=WriteModeChk(HK_MemYN,"location.href='?mode=reply&idx="& Idx &"&Ref="& Ref &"&ReLevel="& ReLevel &"&page="& Page &"&"&PageStr&"'",prepage)%>" class="btn-pack medium"> -->
<% End IF %>

<% IF HK_NotYN<>"True" AND ReplyCnt=1 Then %>

	<% IF CStr(status)="0" OR CStr(status)="1" THEN %>
	<% IF Pwd="" Or IsNull(Pwd) THen %>
		<% IF CStr(UserIdx)=CStr(Session("useridx")) AND CStr(Session("useridx"))<>"" Then %>
		<li><a href="javascript:goMemberCheck('del','<%=idx%>');" class="click cancel"><%=LangPack_wBtnDel%></a></li>
		<% End IF %>
	<% Else %>
		<li><a href="javascript:goPwdpage('bbsdel',<%=idx%>);" class="click cancel"><%=LangPack_wBtnDel%></a></li>
	<% End IF %>
	<% IF Pwd="" Or IsNull(Pwd) THen %>
		<% IF CStr(UserIdx)=CStr(Session("useridx")) AND CStr(Session("useridx"))<>"" Then %>
		<li><a href="javascript:goMemberCheck('modify','<%=idx%>');" class="click point"><%=LangPack_wBtnModify%></a></li>
		<% End IF %>
	<% Else %>
		<li><a href="javascript:goPwdpage('bbsmodify',<%=idx%>);" class="click point"><%=LangPack_wBtnModify%></a></li>
	<% End IF %>
	<% End IF %>
<% End IF %>
