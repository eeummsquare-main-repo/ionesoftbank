...
$(document).ready(function(){
	...
	var table = new Tabulator("#example-table", {
		...
		<% IF GB_ADMEMSORT="0" OR GB_ADMEMSORT="1" Then %>
		selectableRange:1,
		...
		columns:[
			{title:"월", field:"pMonth", width:60, cssClass:"pMonth"<% IF GB_ADMEMSORT="0" OR GB_ADMEMSORT="1" Then %>, editor:"number"<% End IF %>, editorParams:{ selectContents:true, verticalNavigation:"table" }},
			{title:"No", field:"pNo", width:60<% IF GB_ADMEMSORT="0" OR GB_ADMEMSORT="1" Then %>, editor:"number"<% End IF %>, editorParams:{ selectContents:true, verticalNavigation:"table" }},
			{title:"구분", field:"pDiv", width:70, cssClass:"pDiv"<% IF GB_ADMEMSORT="0" OR GB_ADMEMSORT="1" Then %>, editor:"list"<% End IF %>, editorParams:{values:{"구조설계 ":"구조설계 ", "안전진단 및 검토":"안전진단 및 검토", "안전진단":"안전진단", "성능평가 및 검증":"성능평가 및 검증"}}},
			...
			<% IF GB_ADMEMSORT="0" OR GB_ADMEMSORT="1" Then %>
			{title:"견적금액", field:"estAmount", width:90, cssClass:"estAmount"<% IF GB_ADMEMSORT="0" OR GB_ADMEMSORT="1" Then %>, editor:"number"<% End IF %>, editorParams:{ selectContents:true, verticalNavigation:"table" }, formatter:"money", formatterParams:{decimal:".", thousand:",", negativeSign:false, precision:false}},
			{title:"결정금액", field:"setAmount", width:90, cssClass:"setAmount"<% IF GB_ADMEMSORT="0" OR GB_ADMEMSORT="1" Then %>, editor:"number"<% End IF %>, editorParams:{ selectContents:true, verticalNavigation:"table" }, formatter:"money", formatterParams:{decimal:".", thousand:",", negativeSign:false, precision:false}},
			{title:"외주비용", field:"outAmount", width:90, cssClass:"outAmount"<% IF GB_ADMEMSORT="0" OR GB_ADMEMSORT="1" Then %>, editor:"number"<% End IF %>, editorParams:{ selectContents:true, verticalNavigation:"table" }, formatter:"money", formatterParams:{decimal:".", thousand:",", negativeSign:false, precision:false}},
			{title:"외주처", field:"outCompany", width:100},
			<% End IF %>

			<% IF GB_ADMEMSORT="0" Then %>
			{
				title:"수금일 및 수금액",
				headerHozAlign:"center",
				columns:[
			...
			<% End IF %>
			{title:"실적<br>등록", field:"isReg", width:50<% IF GB_ADMEMSORT="0" OR GB_ADMEMSORT="1" Then %>, editor:true<% End IF %>, formatter:"tickCross", resizable:false, cssClass:"lineNone"},
		],
	});
		...
		<div class="contents" style="padding-bottom:0; width:98%; min-width:1280px;">
			...
			<div class="calArea">
				...
				<div style="position:absolute; right:0; bottom:10px;">
					...
					<% End IF %>
					<a id="download-xlsx" class="btn_gray bc_green" style="font-size:14px; width:100px; line-height:20px;">엑셀다운로드</a>
				</div>
			</div>
			...
			<div style='clear:both; padding-bottom:100px'>
				...
				<div class="btminfo">
					...
					<p class="won">
						프로젝트수 : <span id="tProjectCnt">0</span>
						/ 결정금액 : <span id="tEstPrice">0</span>
						<% IF GB_ADMEMSORT="0" Then %>
						/ 수금금액 : <span id="tSuKumPrice">0</span>
						<% End IF %>
					</p>
				</div>
			</div>
